...
#         else: showLegend=False
        ...
        ax,ks_p_cohort1_cohort2,t_p_cohort1_cohort2,p_Anov,filename=plotHistComprison(dataList,ax,
                                    title,showLegend=False,nBins=20,toAnnotate=False,alpha=None,plotType='hist',
...
# 		ax.tick_params(axis='both', direction='in')
        ax.legend([],[])
#         ax.set_title(feature_name_list[n],fontsize='medium',y=-0.005)
        ax.tick_params(axis='both',direction='in')
...
# load feature values
Xfeatures_file = DATA_DIR + "TCRfeatures/TCRfeaturesNoT_\
PCA10percShared_withRels_VDJ_noCorr0-999_nanFilled_noConsts.xlsx"
Xfeatures = (pd.read_excel(Xfeatures_file).set_index('BD').loc[PNP530, :])
...
features_df.index.rename('index',inplace=True)
jitter = 0.1
sample_num = None
scalingMethod = 'perc'
target_name = 'isCardio'

ax4,fig = gen_shap_summary_to_axes(ax4, fig,target_name, nTopFeatures, shap_df, features_df,
...
fig.savefig(FIG2_DIR + 'figure2_%s_ver2.png' % cdate, dpi=300)
