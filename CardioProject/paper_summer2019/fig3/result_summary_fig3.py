...
############################################################################################
#  Generation of result excel files for figure 3
############################################################################################
...
direction_col = 'Associated_with_isCardio_positive'
top100_clusters = convert_logp_values(top100_clusters,direction_col,logp_col='-log10p_MW',suffix='cluster_isCardio')
p_value_col = [col for col in top100_clusters.columns if 'p_to_show' in col][0]
top100_clusters_order = (top100_clusters.sort_values(by=p_value_col))['cluster'].tolist() #get the order of clusters
                                                                                                    # based on p-value magnitutde and sign
...
top100_clusters_with_annot = top100_clusters_with_annot.set_index('cluster').loc[top100_clusters_order,:]
...
cluster_species_hm = cluster_species_hm.loc[top100_clusters_order,col_order]
...
top100_phen_df = top100_phen_df.loc[top100_clusters_order,:]
...
df_list = [top100_clusters_with_annot,cluster_species_hm, top100_phen_df]
...
cluster_df_for_fig = cluster_df.loc[sample_list1+sample_list2, top100_clusters_order]
